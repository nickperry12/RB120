...
We are getting an error thrown stating a private method has been called on an
`Employee` object. The error states that it's occuring on line 63 within the
`Employee#==` method. This method attempts to call the `Employee#serial_number`
method on an `Employee` object being passed in as an argument. However, because
`serial_number` is placed below a `private` method call, it's only available to
other instance methods of that class. 

To fix this, we can change the `private` method call to a `protected` method
call. This will make the `serial_number` method available to other instances of
the class as well.
