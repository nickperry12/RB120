...
You can also define a ` != ` method but we need to be careful about it. It wouldn't make sense if two objects could be bot equal and not equal simultaneously, which is possible if you override both ` == ` and ` != ` .
